...
namespace Ownership.Application.CommandHandler
{
    public class OwnerCommands : IOwnerCommands
    {
        ...
        public OwnerCommands(IUnitOfWork unitOfWork , IMapper mapper, IDomainEventPublisher domainEventPublisher)
        {
        ...
        public async Task<Result<OwnerResponse>> AddOwnerAsync(Guid id, string email, string name, string address, string contactNumber, CancellationToken cancellationToken)
        {
            ...
            await _domainEventPublisher.PublishAsync(owner.DomainEvents, default);
